...
do 
	...
	do 
		LOG_FILE=sigmod19_experiments_set1_$dataset_$model_monetdb.log

		echo " MonetDB -- sql -- $dataset -- $model" >> runtime/"$LOG_FILE"

		./multifaq --path ../benchmarking/datasets/"$dataset" --model "$model" --codegen sql --feat config_sigmod19/features_"$model".conf >> log_multifaq_sigmod19_experiments_set1_$dataset_$model_monetdb.log
		...
		for i in {1..5}
		do
		      echo "Run $i $SQL_SCRIPT  - MonetDB - $dataset - $model" >> ../../log_multifaq_sigmod19_experiments_set1_$dataset_$model_monetdb.log
