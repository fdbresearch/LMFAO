...
for dataset in retailer favorita tpc-ds; 
do 
	for model in perc; ##covar count rtree mi cube;
	do 
		LOG_FILE=sigmod19_experiments_set1_"$dataset"_"$model".log

		./multifaq --path ../benchmarking/datasets/"$dataset" --model "$model" --parallel "$PARALLEL" --feat config_sigmod19/features_bestroot.conf >> log_multifaq_sigmod19_experiments_set1_"$dataset"_"$model".log
